...
**Presentation enable:** `GET /api/draft/enable?sanity-preview-secret=<generated-by-sanity>&redirect=/path`
**Disable:** `GET /api/draft/disable`

When draft mode is enabled, Sanity fetches use `previewClient` (no CDN, `perspective: "drafts"`, authenticated with `SANITY_API_READ_TOKEN`). The Sanity Presentation tool uses the `sanity-preview-secret` flow generated by Sanity, while legacy preview links still use `SANITY_PREVIEW_SECRET`. Draft previews render Next Sanity Visual Editing overlays when draft mode is active.
...
| `NEXT_PUBLIC_SANITY_STUDIO_URL` | For visual editing | Sanity preview client | Hosted Studio URL used by stega/source-map links |
| `SANITY_STUDIO_PREVIEW_ORIGIN` | For deployed Studio previews | Sanity Studio build | Frontend origin loaded inside the Presentation tool |
